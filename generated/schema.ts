...
export class StakingPool extends Entity {
  ...
  get stakingPool(): string | null {
    let value = this.get("stakingPool");
    if (!value || value.kind == ValueKind.NULL) {
      return null;
    } else {
      return value.toString();
  ...
  set stakingPool(value: string | null) {
    if (!value) {
      this.unset("stakingPool");
    } else {
      this.set("stakingPool", Value.fromString(<string>value));
    }
  }
}
